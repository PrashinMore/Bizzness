...
Before using CRM features, you must check if CRM is enabled for the organization. Organization settings control feature availability and behavior.
...
```json
{
  ...
  "businessLogo": "/uploads/business/logo.jpg",
...
### CRM-Related Settings

The following settings control CRM functionality:

- **`enableCRM`** (boolean): Master switch for CRM features
  - If `false`, all CRM endpoints will return `403 Forbidden`
  - Must be `true` to use any CRM functionality

- **`enableLoyalty`** (boolean): Controls loyalty program features
  - If `true`, loyalty points are automatically awarded on purchases
  - Points are calculated as: `floor(billAmount / 100)`
  - Tiers: Silver (0-499), Gold (500-999), Platinum (1000+)
...
interface OrganizationSettings {
  ...
  // CRM Settings
  enableCRM: boolean;            // Master CRM switch
  enableLoyalty: boolean;        // Loyalty program switch
...
**Best Practice**: Always check `enableCRM` before making CRM API calls:

```typescript
// Example: Check if CRM is enabled
async function checkCrmEnabled(token: string): Promise<boolean> {
  try {
    ...
    return settings.enableCRM === true;
  } catch (error) {
...
// Usage
if (await checkCrmEnabled(token)) {
  // Show CRM features, make CRM API calls
} else {
  // Hide CRM features, show message or upgrade prompt
}
```

### Updating CRM Settings

**Note**: Only organization administrators can update settings. CRM settings are typically managed through the web dashboard, but mobile apps can read them to control feature visibility.
